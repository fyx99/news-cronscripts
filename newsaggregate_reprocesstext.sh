...
docker exec postgres psql -U postgres -d newsaggregate -c "INSERT INTO Runs (task, exit_code) values ('REPROCESS_TEXT', '`docker container inspect -f '{{.State.ExitCode}}' ${containerid}`');"
